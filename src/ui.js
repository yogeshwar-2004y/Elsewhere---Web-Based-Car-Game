import { createIcons, Volume2, VolumeX, Maximize, SlidersHorizontal, ArrowUpRight, Compass, Check, Sun, Moon, Route, Shuffle, ChevronUp, ChevronDown, Infinity, MountainSnow, X, Headphones, Gamepad2, Link, Play, Pause, ArrowLeft, ArrowRight, ChevronsUp } from 'lucide';
const icons={Volume2,VolumeX,Maximize,SlidersHorizontal,ArrowUpRight,Compass,Check,Sun,Moon,Route,Shuffle,ChevronUp,ChevronDown,Infinity,MountainSnow,X,Headphones,Gamepad2,Link,Play,Pause,ArrowLeft,ArrowRight,ChevronsUp};
...
export function mountUI() {
  ...
  <div class="drive-label" id="drive-label"><span class="live-dot"></span><span id="location">THE QUIET SIDE OF THE ALPS</span></div>
  ...
  <div class="bottom-bar">
    ...
    <div class="drive-hud" id="drive-hud"><div class="speed"><strong id="speed">0</strong><span>km/h</span></div><div class="hud-divider"></div><div class="distance"><strong id="distance">0.0 <span>km</span></strong><small>of taking it easy</small></div><button id="pause-btn" class="icon-button" aria-label="Pause drive">${icon('pause')}</button></div>
  ...
  <div class="car-picker popover" id="car-picker" hidden><div class="popover-title"><span>Find your companion</span><button class="close-small" data-close="car-picker" aria-label="Close car picker">${icon('x')}</button></div>${[['lamborghini','Lamborghini','Bold spirit. Effortless power.','#d4df69'],['bmw','BMW 2002','Old soul. Open road.','#e88c50'],['porsche','Porsche 911','Made for the winding way.','#d8ddd0']].map(([id,name,sub,color])=>`<button data-car="${id}" class="car-option ${id==='bmw'?'selected':''}" aria-pressed="${id==='bmw'}"><span class="paint-chip" style="--paint:${color}"></span><span><strong>${name}</strong><small>${sub}</small></span>${icon('check')}</button>`).join('')}</div>
  <div class="modal-backdrop" id="settings-modal" hidden><section class="dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title"><div class="dialog-heading"><div class="eyebrow">THE LITTLE THINGS</div><button class="icon-button" data-close="settings-modal" aria-label="Close settings">${icon('x')}</button></div><h2 id="settings-title">Settle in.</h2><p>A few adjustments for your kind of drive.</p><label class="setting-row">Graphics quality<select id="quality"><option value="high">High · scenic</option><option value="medium">Medium · balanced</option><option value="low">Low · easygoing</option></select></label><label class="setting-range">Sound volume <output id="volume-output">32%</output><input id="volume" aria-label="Sound volume" type="range" min="0" max="100" value="32"></label><label class="setting-range">Ambient music <output id="music-output">30%</output><input id="music" aria-label="Ambient music" type="range" min="0" max="100" value="30"></label><div class="settings-note">${icon('headphones')} Prefer your own soundtrack? Turn the music down and put on something you love.</div><div class="controls-heading">A FEW FRIENDLY DIRECTIONS</div><div class="controls-grid"><span>Accelerate / brake</span><b>W / S or ↑ / ↓</b><span>Steer</span><b>A / D or ← / →</b><span>Return to road</span><b>R</b><span>Autodrive</span><b>F</b><span>Camera / day & night</span><b>C / N</b><span>Pause / hide interface</span><b>Space / H</b></div><div class="gamepad-note" id="gamepad-note">${icon('gamepad-2')} Gamepad ready · left stick + triggers</div><button class="primary-button dialog-done" data-close="settings-modal">Back to the open road ${icon('arrow-up-right')}</button></section></div>
