import { multiplayerMarkup } from './multiplayer-ui.js';
import { CAR_SPECS } from './dynamics.js';
import { createIcons, Volume2, VolumeX, Maximize, SlidersHorizontal, ArrowUpRight, Compass, Check, Sun, Moon, Route, Shuffle, ChevronUp, ChevronDown, Infinity, MountainSnow, X, Headphones, Gamepad2, Link, Play, Pause, ArrowLeft, ArrowRight, ChevronsUp, Camera, Users, Copy } from 'lucide';
const icons={Volume2,VolumeX,Maximize,SlidersHorizontal,ArrowUpRight,Compass,Check,Sun,Moon,Route,Shuffle,ChevronUp,ChevronDown,Infinity,MountainSnow,X,Headphones,Gamepad2,Link,Play,Pause,ArrowLeft,ArrowRight,ChevronsUp,Camera,Users,Copy};
...
export function mountUI() {
  ...
  <header class="topbar">
    ...
    <nav class="top-actions" aria-label="Game tools">
      <button class="icon-button" id="multiplayer-btn" title="Drive with friends" aria-label="Open multiplayer">${icon('users')}</button>
  ...
  <section class="intro" id="intro">
    ...
    <button id="start-btn" class="primary-button">Drive solo ${icon('arrow-up-right')}</button>
<button id="friends-btn" class="friends-button">${icon('users')} Or drive with friends ${icon('arrow-up-right')}</button>
